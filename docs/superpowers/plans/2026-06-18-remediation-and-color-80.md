...
- [x] **Session 3 — P1 runtime + P0 an ninh + chốt màu** · WS-SECEVAL ✅ `e41d26e` · màu 86% deployed ✅ `150e59c` · WS-RUNTIME ✅ (O2/E3 qua `build_pipeline` unification `e6f1974`/`9d88608`; E1/E2 chốt `c5bc60d`).
- [x] **Session 4 — P2 polish + nghiệm thu** · WS-REPRO ✅ (R2 `9a1d252`, R3 db=20 xe, R1/E4/O3 chốt `63aceba`) · full verification 2026-07-10: pytest 98/7, benchmark 86.28% TTA tái lập, E2E dashboard OK.
...
- 2026-06-19 — **Cải thiện Report 3 & 4 (user đã nộp Report 2).** R3 §5: thêm bảng kết quả **Plate detector** (Benchmark B, mAP50 0.9896) + **OCR** (Benchmark C, PaddleOCR 0.812 vs EasyOCR 0) → cả 4 thành phần đều có kết quả §5 (commit `65c9d3d`). R4: sửa color 55%→86% (§6), annotate §4.1 (màu từ model cũ), sơ đồ §3 bỏ brand. **WS-SECEVAL XONG (commit `e41d26e`):** `main/scripts/eval_security.py` đo chống tráo biển trên model ĐANG DEPLOY + `verify_vehicle` thật, 889 ảnh VCoR held-out (600 trial): **phát hiện tráo biển 98.5%, false-alarm 14.5%, unregistered 100%** → Report 4 §4.3 + `docs/benchmarks/security_eval.md` (lần đầu có số an ninh định lượng cho mục tiêu ≥95% của Report 1). Limitation trung thực: tráo CÙNG màu không bắt được, phụ thuộc OCR, domain VCoR→CCTV. pytest 44/7.
  **CÒN cho Report 4:** §4.1 chạy lại E2E 5 ảnh với model màu mới (sẽ lộ domain gap thật trên ảnh CCTV — trung thực). Ngoài ra WS-RUNTIME, WS-REPRO vẫn mở.
- 2026-07-10 — **Session 3+4 XONG — TOÀN BỘ PLAN HOÀN TẤT.** Kiểm kê đầu phiên: phần lớn Session 3/4 đã được các phiên không-log (20/06–03/07) làm xong — O2+E3 (API & dashboard dùng chung `build_pipeline`, PaddleOCR-only, brand diagnostic-only: `e6f1974`, `9d88608`), Report 4 §4.1 đã chạy lại với model 86% (`12a0b9e`), R2 Docker (paddle 2.x pins, zero-network: `9a1d252`), R3 (database.csv = 20 xe). Phiên này chốt phần còn thiếu qua Sonnet: **E2** `_collect` log exception thay vì nuốt im (TDD caplog, giữ skip-frame semantics) + **E1** docstring `_select_device` trung thực + xoá import Keras/Torch chết & comment sai "Keras out-of-process" trong dashboard.py → `c5bc60d`; **R1** torch/torchvision explicit + **E4** run_evaluation.py PROJECT_ROOT (hết phụ thuộc CWD) + **O3** 2 test edge row-cluster `/3` → `63aceba`. Verify (Fable): (1) benchmark protocol `eval_color_deployed.py` chạy lại trên VCoR held-out 889 ảnh → **85.26% plain / 86.28% TTA, macro-F1 0.84** — byte-identical với report đã commit (git diff rỗng, model runtime đúng là bản 86%); (2) pytest **98 passed / 7 skipped / 0 failed** (= 87 baseline + 8 của phiên conf-slider song song `77ca8f4..d2cbd17` + 3 mới); (3) E2E dashboard (autoPort vì phiên khác giữ :8502): sidebar **Models loaded 3/3, ● color_clf**, upload `test_authorized.jpg` → **51F06532 · Colour: Black (38.1%) · AUTHORIZED** (màu đúng, conf thấp = domain gap CCTV đã ghi trong report); (4) grep 4 reports + README: hết mâu thuẫn, ResNet50 chỉ còn dạng chú thích lịch sử có annotate. Lưu ý: 38.1% conf trên CCTV nhắc lại caveat domain — hướng cải thiện (white-balance + data CCTV) đã ghi ở Report 3/4, ngoài scope plan này.
