...
|---|---|---|---|
| **Plate-swap detection** (headline) | 200 | detection rate (color_warning=True) | **56.5%** (113/200) |
| Plate-swap MISSED | 200 | miss rate | 43.5% (87/200) |
| Legitimate (no swap) | 200 | false-alarm rate | 0.5% (1/200) |
| Unregistered plate | 200 | detection rate (DENY_ALERT) | 100.0% (200/200) |

- **Plate-swap detection rate = 56.5%** — khi biển số bị tráo lên xe KHÁC MÀU, hệ thống bắt được 113/200 lần qua cảnh báo màu lệch.
- **False-alarm rate = 0.5%** — xe hợp lệ (không tráo) bị cảnh báo nhầm 1/200 lần (do model màu dự đoán sai ngay cả khi biển đúng).
...
| Brown | Silver | 4 | 3 | 75.0% |
| Red | Silver | 5 | 3 | 60.0% |
| Silver | Brown | 6 | 3 | 50.0% |
| Blue | Grey | 2 | 2 | 100.0% |

**Cụm màu trung tính (Black/Grey/Silver/White ↔ nhau):** 50 trial, miss 49 (98.0%) — khớp với cụm nhập nhằng đã ghi nhận ở Report 3 §5.1 (confusion matrix màu), đây là nơi cross-check màu YẾU NHẤT vì model màu chính nó cũng nhầm trong cụm này.
...
_Sinh bởi `main/scripts/eval_security.py`, chạy lúc thực thi script này trên model đang triển khai (`/Users/konalyn/Documents/FPT Materials/DPL302m/PDL302m_project/main/data/models/color_MobileNetV3Small.pt`), 600 trial tổng, elapsed 8.3s._
