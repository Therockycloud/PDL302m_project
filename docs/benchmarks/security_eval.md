...
|---|---|---|---|
| **Plate-swap detection** (headline) | 200 | detection rate (color_warning=True) | **98.5%** (197/200) |
| Plate-swap MISSED | 200 | miss rate | 1.5% (3/200) |
| Legitimate (no swap) | 200 | false-alarm rate | 14.5% (29/200) |
| Unregistered plate | 200 | detection rate (DENY_ALERT) | 100.0% (200/200) |

- **Plate-swap detection rate = 98.5%** — khi biển số bị tráo lên xe KHÁC MÀU, hệ thống bắt được 197/200 lần qua cảnh báo màu lệch.
- **False-alarm rate = 14.5%** — xe hợp lệ (không tráo) bị cảnh báo nhầm 29/200 lần (do model màu dự đoán sai ngay cả khi biển đúng).
...
|---|---|---|---|---|
| Grey | Brown | 2 | 1 | 50.0% |
| Silver | Grey | 5 | 1 | 20.0% |
| Grey | Silver | 5 | 1 | 20.0% |

**Cụm màu trung tính (Black/Grey/Silver/White ↔ nhau):** 50 trial, miss 2 (4.0%) — khớp với cụm nhập nhằng đã ghi nhận ở Report 3 §5.1 (confusion matrix màu), đây là nơi cross-check màu YẾU NHẤT vì model màu chính nó cũng nhầm trong cụm này.
...
_Sinh bởi `main/scripts/eval_security.py`, chạy lúc thực thi script này trên model đang triển khai (`/Users/konalyn/Documents/FPT Materials/DPL302m/PDL302m_project/main/data/models/color_MobileNetV3Small.pt`), 600 trial tổng, elapsed 8.8s._
